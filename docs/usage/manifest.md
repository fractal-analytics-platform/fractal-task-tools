...
`fractal-task-tools` includes a set of [task models](../code_reference/task_models.md) to be used in the `task_list.py` module. An example may look like
```python
...
The list of forbidden values is as follows (note: this is currently evolving, and the ground truth is in the [`_specs.py` module](../../code_reference/_specs/#fractal_task_tools._specs.validate_schema)):

- E01: Property names cannot be in the [forbidden ones](../../code_reference/_specs/#fractal_task_tools._specs._FORBIDDEN_NAMES).
...
- E13: `anyOf` of [non-`null` primitive types](../../code_reference/_specs/#fractal_task_tools._specs._NON_NULL_PRIMITIVE_TYPES) are not supported.
